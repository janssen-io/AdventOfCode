...
module Day9 =
    ...
    type Parameter =
        | AbsolutePosition of int64
        | RelativePosition of int64
        | Immediate of int64

    type ParameterConstructor = int64 -> Parameter
    ...
    let parseInstruction instruction =
        ...
        | opcode when opcode < 100 ->
            ...
            let modes = List.replicate (arity operation) AbsolutePosition
            operation, modes
        | _ ->
            ...
            if arity operation > 0 then
                let fillerModes = (List.replicate (arity operation - List.length modes) AbsolutePosition)
    ...
    let evalParameter (state: State) (memory:Memory) p =
        match p with
        | Immediate p -> p
        | AbsolutePosition p -> memory.[p]
        | RelativePosition p -> memory.[state.RelativeBase + p]
    ...
    let evalInstruction (instruction, (parameters:Parameter list)) state =
        ...
        let writeMem state param value = 
            ...
            | RelativePosition pos ->
                let memory = Array.copy state.Memory
                memory.[state.RelativeBase + pos] <- value
                { state with Memory = memory; RelativeBase = state.RelativeBase + pos }
        ...
        let readMem state param =
            ...
            | RelativePosition pos ->
                { state with Output = state.Memory.[state.RelativeBase + pos] :: state.Output; RelativeBase = state.RelativeBase + pos }
        ...
        let f () = 
            ...
            |> match instruction with
                | Add -> 
                    List.map (evalParameter state state.Memory)
                    >> List.fold (+) 0
    ...
    [<Fact>]
    let ``Day 7, part one: example`` () =
        let input = [|3;15;3;16;1002;16;10;16;1;16;15;15;4;15;99;0;0|]
        let signal = sendSignal' input [4;3;2;1;0]
        test <@ 43210 = signal @>

    [<Fact>]
    let ``Day 7, part two: basic example`` () =
    ...
    [<Fact>]
    let ``Day 9, part one: example`` () =
        let input = [|1102;34915192;34915192;7;4;7;99;0|]
        let result = run (startState input)
        test <@ result.Memory.[result.InstructionPointer] = 0 @>

    [<Fact>]
    let ``Day 9, part one, input`` () =
        let answer = runPartOne (readinput ())
        test <@ 0 = answer @>

    [<Fact>]
    let ``Day 9, part two: answer`` () =
